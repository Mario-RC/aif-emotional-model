...
The final RLAIF datasets are published on Hugging Face:
...
Full end-to-end reproduction from scratch is possible in principle, but it is not free. Several stages call external LLM APIs or require running local model inference over large models. Anyone attempting to reproduce the entire pipeline should expect monetary cost, GPU time and possible non-determinism. Re-running the pipeline with the same scripts should produce comparable final datasets and metrics, but not necessarily byte-identical files, because API models, generation backends, CUDA kernels, package versions and decoding details can change.
...
### Full approximate reproduction order
...
```bash
cd phase2-sft-alignment/sft-model/sft-demonstration-dataset
python 1-generate_sft_demonstration_dataset.py
python 2-formatted_sft_demonstration_dataset.py
...
python 2-formatted_rm_prompt_dataset.py
python 3-train_test_rm_prompt_dataset_json.py
python 4-merge_demonstration_prompt_datasets.py
```

4. Generate RM candidate responses and curate the RM comparison dataset:
...
python 1-embeddings_semantic_similarity_rm_comparison_dataset.py
python 2-delete_respones_rm_comparison_dataset.py
python 3-add_negative_samples_rm_comparison_dataset.py
python 1-embeddings_semantic_similarity_rm_comparison_dataset.py --test
python 2-delete_respones_rm_comparison_dataset.py --test
python 3-add_negative_samples_rm_comparison_dataset.py --test
```

5. Generate RM preference data. `2-generate_rating_data.py` calls external judge models and may cost money:
...
python 1-preprocess_rm_preference_dataset.py --test
python 2-generate_rating_data.py --test
python 3-rates_to_ranks.py --test
python 4-postprocess_rm_preference_dataset.py --test
python 5-format_rm_preference_dataset.py --test
```

6. Train and evaluate reward models:
...
8. Generate DPO candidate responses and curate the DPO comparison dataset:
...
cd ../candidate-curation
python 1-embeddings_semantic_similarity_dpo_comparison_dataset.py
python 2-delete_respones_dpo_comparison_dataset.py
python 3-add_negative_samples_dpo_comparison_dataset.py
python 1-embeddings_semantic_similarity_dpo_comparison_dataset.py --test
python 2-delete_respones_dpo_comparison_dataset.py --test
python 3-add_negative_samples_dpo_comparison_dataset.py --test
```

9. Generate DPO preference data. `2-generate_rating_data.py` calls external judge models and may cost money:
...
python 1-preprocess_dpo_preference_dataset.py --test
python 2-generate_rating_data.py --test
python 3-rates_to_ranks.py --test
python 4-postprocess_dpo_preference_dataset.py --test
python 5-format_dpo_preference_dataset.py --test
```

10. Train and evaluate the final RLAIF/DPO/PPO/SFT-DPR models:
...
If the API-generated rating CSVs are not present under `data/<LLM>/`, `2-generate_rating_data.py` must be rerun with valid judge API access.
...
This repository follows a minimal-public-artifact policy. The general rule is:

- Keep reusable source code, non-secret configs, launchers and small canonical manifests.
- Keep final public datasets and model releases on Hugging Face.
- Do not version generated `logs/`, `__pycache__/`, temporary notebooks, model checkpoints or large `saves/` outputs.
- Do not assume that every intermediate prediction output or paid API artifact is present in a fresh clone. If it is absent, regenerate it by following the workflow order above.
...
- `phase3-rlaif-alignment/rlaif-model/dpo-preference-dataset/data/<LLM>/dpo_preference_dataset_rate_<LLM>*.csv` is generated by `2-generate_rating_data*.py`; if absent, it must be regenerated with judge API access.
