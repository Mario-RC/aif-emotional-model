...
def check_repeated_dids(data: list[dict]) -> set[str]:
    """Return the set of ``did`` values that occur more than once in ``data``."""
    did_list = [entry["did"] for entry in data]
    return {did for did in did_list if did_list.count(did) > 1}
