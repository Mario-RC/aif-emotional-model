...
def _model_data_path(model: str, suffix: str, is_test: bool) -> str:
    ...
    base = f"data/{model}/rm_comparison_dataset_{short}{suffix}"
    return with_suffix(base, "csv", is_test)


# ---------------------------------------------------------------------------
# Stage 1: convert per-model JSON → CSV
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 2: split emotions / utterances per response
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 4: pairwise semantic similarity + distinct-n
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 5: pick chosen response by combined score
# ---------------------------------------------------------------------------

def _candidate_score_for_row(df: pd.DataFrame, idx: int) -> tuple[list[float], int, str]:
    """Score predict_sft_1..4 (lower idx = recent SFT) and pick the best one."""
