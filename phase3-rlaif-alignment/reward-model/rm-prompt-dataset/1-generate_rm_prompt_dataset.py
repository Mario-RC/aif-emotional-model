"""Generate the seed RM Prompt Dataset CSVs by querying Azure-OpenAI for emotional dialogues.

For each topic in :data:`_lib.TOPICS`, ask the LLM to produce two 4-turn dialogues
with the requested emotion sequence. Checkpoints are written every 30 entries to
``data/rm_prompt_dataset_checkpoint_<n>.csv`` and the final result lands in
``data/rm_prompt_dataset_completions.csv``.
"""
...
def _emotion_block(template_id: int, emotions: list[tuple[str, str]]) -> str:
    """Build the per-dialogue ``(emotion_a) PROMPT`` template block."""
    lines = [f"Dialogue {template_id} emotions:"]
...
def make_messages(topic: str, emotion: list[tuple[str, str]]) -> list[dict]:
    ...
    dialogue_emotions = (
        ...
        f"Provide 2 different new dialogues following the templates below where the topic of the conversations is {topic}.\n\n"
        f"{_emotion_block(1, emotion[:4])}\n\n"
        f"{_emotion_block(2, emotion[4:])}"
    )
    return [
...
def generate_rm_prompt_dataset(
    ...
    for idx, (topic, emotion) in enumerate(tqdm(topics_emotions, start=1), start=1):
        ...
        if idx % CHECKPOINT_INTERVAL == 0:
            ...
            df_chk.to_csv(f"data/rm_prompt_dataset_checkpoint_{chkpt}.csv", index=False)
            chkpt += 1
